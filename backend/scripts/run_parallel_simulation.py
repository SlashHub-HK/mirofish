...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr streams to UTF-8 (fix console encoding issues)
    ...
    # Note: this needs to be set at Python startup; setting at runtime may not take effect
...
# Script is located at backend/scripts/
...
# Load .env file from project root (contains LLM_API_KEY and other settings)
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logging every agent's observations and actions); we use our own action_logger instead.
    """
...
# Available Twitter actions (excludes INTERVIEW; INTERVIEW can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms simultaneously, return combined results
        ...
        # Interview on both platforms in parallel
        tasks = []
...
# Action type mapping table (database names -> standard names)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of codenames like "Agent_0".

    Args:
        config: Contents of simulation_config.json

    Returns:
        Mapping dict of agent_id -> entity_name
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and enrich them with full context information.

    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value from the last read (using rowid instead of created_at because different platforms use different created_at formats)
...
def _enrich_action_context(
    ...
    """
    Enrich an action with context information (post content, usernames, etc.).
    ...
    try:
        ...
        # Follow user: enrich with the followed user's name
        ...
        # Mute user: enrich with the muted user's name
        ...
        # Create comment: enrich with the commented post's information
...
def _get_post_info(
    ...
    Returns:
        Dict containing content and author_name, or None
    """
    try:
        cursor.execute("""
...
def get_active_agents_for_round(
    ...
    """Determine which Agents to activate for this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional, to truncate excessively long simulations)
    ...
    # Twitter uses the standard LLM configuration
    ...
    # If max rounds is specified, truncate
    ...
    log_info(f"Simulation loop completed! Time elapsed: {elapsed:.1f}s, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (if available, otherwise falls back to standard config)
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: after simulation completes, don't exit, wait for interview commands.
    When a termination signal is received:
    1. Notify the asyncio loop to exit the wait
    2. Give the program a chance to properly clean up resources (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (giving the loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
